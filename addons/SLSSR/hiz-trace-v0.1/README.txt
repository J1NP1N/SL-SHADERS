...
It consumes the already-validated `SL Hi-Z v0.1a - Min/Max + Interval Debug` pyramid through `SLHiZTraceLink.addon` and traces reflected rays with coarse min/max tile rejection plus a full-resolution finite depth-slab test.
...
TECHNIQUE ORDER
- SL Hi-Z v0.1a - Min/Max + Interval Debug
- SL SSR v0.35 - Legacy Resolve
- SL SSR Temporal v0.1 - Reprojected History
- SL Hi-Z Trace v0.1 - Depth-Slab Compare   <-- LAST

The Hi-Z tracer's default display is Passthrough, so it changes nothing until a diagnostic mode is selected.
...
2. At the known grazing-ghost camera angle choose `v0.35 vs Hi-Z hit mask`.
