// SL_BackgroundDepth_v0_2.fx
// Diagnostic for Firestorm-native primary + static-background depth.
// v0.2 removes ReShade's generic SL_DEPTH from the comparison entirely.
...
texture SLPrimaryNativeDepthTex : SL_DEPTH_PRIMARY_NATIVE;
texture SLBackgroundDepthTex : SL_DEPTH_BACKGROUND;

sampler SLPrimaryNativeDepthSampler
{
    Texture = SLPrimaryNativeDepthTex;
...
float4 BackgroundDepthDebugPS(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    ...
    bool linkOK =
        ...
        HasExactMatrices() &&
        SLPrimaryDepthNativeSize.x > 1.0 && SLPrimaryDepthNativeSize.y > 1.0 &&
        SLBackgroundDepthSize.x > 1.0 && SLBackgroundDepthSize.y > 1.0;
...
<
    ui_label = "SL Background Depth v0.2 - Native Pair";
    ui_tooltip = "Compares Firestorm-native primary deferred depth against Firestorm-native static background depth. Diagnostic only.";
>
{
    pass Debug
    {
